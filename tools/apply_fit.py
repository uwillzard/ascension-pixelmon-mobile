...
# ---------------------------------------------------------------------
# 13) v0.26 - Controles exatos do launcher Mundo Cobblemon no Ascension.
# ---------------------------------------------------------------------
# Fonte: assets/oficial.json extraído estaticamente do APK fornecido pelo
# usuário. Não redesenha nem converte o layout; preserva o JSON original.
...
_asc_controls_sha = _asc_hashlib.sha256(_asc_controls_bytes).hexdigest()
if _asc_controls_sha != "156b2d4669bff8c9377b73d79941ae6bd44682d356aebba230bd018530ea5491":
    raise SystemExit("[Ascension v0.26] SHA-256 do controle não confere")

bootstrap = bootstrap_path.read_text(encoding="utf-8")

profile_anchor = '        profile.gameDir = AscensionConfig.GAME_DIR_NAME;\n'
profile_line = '        profile.controlFile = "ascension-mundo-controls.json";\n'
if profile_line not in bootstrap:
    if profile_anchor not in bootstrap:
        raise SystemExit("[Ascension v0.26] ponto do controlFile não encontrado")
    bootstrap = bootstrap.replace(profile_anchor, profile_anchor + profile_line, 1)

finish_anchor = '''        File gameDir = ensureLauncherProfile(neoId);
        ensureLocalAccount(nick, neoId);
'''
finish_replacement = '''        File gameDir = ensureLauncherProfile(neoId);
        ensureAscensionMundoControls();
        ensureLocalAccount(nick, neoId);
'''
if "ensureAscensionMundoControls();" not in bootstrap:
    if finish_anchor not in bootstrap:
        raise SystemExit("[Ascension v0.26] finishSetup não encontrado")
    bootstrap = bootstrap.replace(finish_anchor, finish_replacement, 1)

helper_anchor = '    private void ensureLocalAccount(String nick, String selectedVersion) throws Exception {\n'
if "private void ensureAscensionMundoControls()" not in bootstrap:
    helper = r'''    private void ensureAscensionMundoControls() throws Exception {
        File controlDir = new File(Tools.CTRLMAP_PATH);
        if (!controlDir.exists() && !controlDir.mkdirs()) {
            throw new IllegalStateException("não foi possível criar a pasta de controles");
        }

        File target = new File(controlDir, "ascension-mundo-controls.json");
        File temp = new File(controlDir, "ascension-mundo-controls.tmp");
        ...
        if (!temp.isFile() || temp.length() < 1000) {
            temp.delete();
            throw new IllegalStateException("arquivo de controles ficou inválido");
        }

        if (target.exists() && !target.delete()) {
            temp.delete();
            throw new IllegalStateException("não foi possível atualizar os controles");
        }

        if (!temp.renameTo(target)) {
            temp.delete();
            throw new IllegalStateException("não foi possível ativar os controles");
        }

        android.content.SharedPreferences pojavPrefs = LauncherPreferences.DEFAULT_PREF;
        if (pojavPrefs != null) {
            pojavPrefs.edit().putString("defaultCtrl", target.getAbsolutePath()).commit();
        }
...
'''
    if helper_anchor not in bootstrap:
        raise SystemExit("[Ascension v0.26] ponto para helper de controles não encontrado")
    bootstrap = bootstrap.replace(helper_anchor, helper + helper_anchor, 1)

# Corrige as chaves duplicadas usadas apenas para escapar o f-string deste hotfix.
bootstrap = bootstrap.replace("throws Exception {", "throws Exception {")  # no-op intencional

bootstrap_path.write_text(bootstrap, encoding="utf-8")
print("[Ascension v0.26] controles Mundo Cobblemon oficiais -> perfil Ascension Pixelmon OK")
